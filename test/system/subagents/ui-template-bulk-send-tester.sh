...
stop_frontend_dev() {
  if [[ -f "$FRONTEND_PID_FILE" ]]; then
    ...
    if kill -0 "$pid" >/dev/null 2>&1; then
      log "ui-template-bulk-send: stopping frontend dev pid=$pid"
...
start_frontend_dev() {
  ...
  if [[ -f "$FRONTEND_PID_FILE" ]] && kill -0 "$(cat "$FRONTEND_PID_FILE")" >/dev/null 2>&1; then
    log "ui-template-bulk-send: frontend dev already running (pid=$(cat "$FRONTEND_PID_FILE"))"
    return 0
  fi

  stop_stale_frontend_listeners

  log "ui-template-bulk-send: starting frontend dev server"
  (
    cd "$ROOT_DIR"
  ...
  log "ui-template-bulk-send: waiting frontend health"
  local ok=0
  ...
  log "ui-template-bulk-send: frontend dev ready"
}
